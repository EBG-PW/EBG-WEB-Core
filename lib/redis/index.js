...
/**
 * Get all Netdata Overviews Objects from Redis
 * @param {array} monitors - The array of monitors
 * @returns {Promise<Array>}
 */
const readNetdataOverviews = async (monitors) => {
    return new Promise(async (resolve, reject) => {
        for (let monitor of monitors) {
            // Data we get is gziped buffer
            const data = await redis.get(`NDM:${monitor.ip_address}:${monitor.hostname}:OVERVIEW`);
            if (data) monitor.overview = Buffer.from(data, 'base64');
        }
        resolve(monitors);
    })
}
...
const Netdata = {
    getOverviews: readNetdataOverviews
}
