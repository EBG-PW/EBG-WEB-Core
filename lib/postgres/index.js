...
const WebtokensGet = (token) => {
  return new Promise((resolve, reject) => {
    // Join the users table with the webtokens table
    pool.query(`SELECT * FROM webtokens INNER JOIN users ON webtokens.user_id = users.id WHERE token = $1`, [token], (err, result) => {
      if (err) { reject(new SQLError(err)) }
      resolve(result.rows)
    })
  })
