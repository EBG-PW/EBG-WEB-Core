  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (users.length === 0) {
        ...
        await client.query(`INSERT INTO users_settings (user_id, language, design) VALUES ($1, $2, $3)`, [newUserInsertResult.rows[0].id, 'en', 'white.center']);
      } else {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (users.length === 0) {
        ...
        await client.query(`INSERT INTO users_settings (user_id, language, design) VALUES ($1, $2, $3)`, [newUserInsertResult.rows[0].id, locale, 'white.center']);
      } else {
        // If user exists, update fields that are null
        const user = users[0];
