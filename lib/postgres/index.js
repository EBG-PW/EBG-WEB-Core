...
/**
 * Get the OAuth Token Data based on the access token and its scope
 */
const getAuthorizedOauthData = async (accessToken) => {
  const client = await pool.connect();
  try {
    const scopeQuery = `
    ...
    `;
    const scopeResult = await client.query(scopeQuery, [accessToken]);
