...
const getServerMonitorDataByUserID = async (user_id, page, size, search) => {
  try {
    const res = await pool.query(
      `SELECT ip_address, hostname, charts FROM netdata_monitoring WHERE user_id = $1 AND hostname ILIKE $2 OFFSET $3 LIMIT $4`,
...
const srvmon = {
  ...
  countByID: getServerMonitorDataCountByUserID,
  getByID: getServerMonitorDataByUserID
}
