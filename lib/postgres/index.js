...
const createTables = async () => {
  ...
  await createTable(`CREATE TABLE IF NOT EXISTS users_integrations (
      ...
      PRIMARY KEY (user_id, platform),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);`, "users_thirdparty")
...
const UsersCreate = (username, email, password, language, design, user_group, avatar_url, first_name, last_name, bio) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const returnVal = await client.query(`INSERT INTO users (username, email, password, user_group, avatar_url, first_name, last_name, bio) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [username, email, password, user_group, avatar_url, first_name, last_name, bio]);
...
const getOAuthClientApplicaiton  = async (client_id, scope, user_id) => {
  ...
  try {
    ...
    const appQuery = `SELECT name, avatar_url, scope, client_id, secret, redirect_url FROM oauth_apps WHERE client_id = $1`;
