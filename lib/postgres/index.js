 ...
 * @returns {Promise<Number|Boolean>} Returns the user ID if a new user was created, otherwise false
 */
...
/**
 * Get all webtokens of a user and flag the row that is the current session
 * @param {Number} user_id 
 * @param {String} current_webtoken
 * @returns 
 */
const GetWebtokensByUserID = async (user_id, current_webtoken) => {
  try {
    const res = await pool.query(
      `SELECT 
        ...
        time, 
        CASE 
          WHEN token = $2 THEN true 
          ELSE false 
        END AS current_session 
      FROM webtokens 
      WHERE user_id = $1`,
      [user_id, current_webtoken]
    );
    return res.rows;
  } catch (error) {
...
/**
 * Delete a webtoken of a user by its uuid
 * @param {String} uuid 
 * @returns 
 */
const DeleteWebtokenByUUID = async (uuid) => {
  try {
    const res = await pool.query(`DELETE FROM webtokens WHERE uuid = $1 returning token`, [uuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Delete all webtokens of a user except the current one
 * @param {Number} user_id 
 * @param {String} current_webtoken 
 * @returns 
 */
const DeleteAllOtherWebtokens = async (user_id, current_webtoken) => {
  try {
    const res = await pool.query(`DELETE FROM webtokens WHERE user_id = $1 AND token != $2`, [user_id, current_webtoken]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Get all users declared by the filter parameter that could be a username or email
 * @param {String} filter
 * @returns {Promise<Array>}
 */
const AdminGetTotalUsersCount = (filter) => {
  return new Promise((resolve, reject) => {
    pool.query(`SELECT COUNT(*) FROM users WHERE username ILIKE $1 OR email ILIKE $1`, ['%' + filter + '%'], (err, result) => {
      if (err) { reject(err) }
...
/**
 * Get all users by page and filter orderd by sort
 * @param {Number} page 
 * @param {Number} size 
 * @param {String} filter 
 * @param {String} sort
 ...
 * @returns {Promise<Array>}
 */
const AdminGetUsersByPage = (page, size, filter, sort, order) => {
  const offset = page * size;
  return new Promise((resolve, reject) => {
    pool.query(`SELECT avatar_url, puuid, username, email, first_name, last_name, user_group FROM users WHERE username ILIKE $1 OR email ILIKE $1 ORDER BY ${sort} ${order} OFFSET $2 LIMIT $3`, ['%' + filter + '%', offset, size], (err, result) => {
      if (err) { reject(err) }
      resolve(result.rows)
    })
  })
 ...
 * @returns 
 */
const AdminGetUserByUUID = (puuid) => {
  return new Promise((resolve, reject) => {
    pool.query(`SELECT * FROM users WHERE puuid = $1`, [puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result.rows)
    })
  })
...
/**
 * Update a user's username by UUID
 * @param {String} puuid 
 * @param {String} username 
 * @returns 
 */
const AdminUpdateUserName = async (puuid, username) => {
  try {
    const res = await pool.query(`UPDATE users SET username = $1 WHERE puuid = $2`, [username, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Update a user's email by UUID
 * @param {String} puuid 
 * @param {String} first_name 
 * @returns 
 */
const AdminUpdateUserFirstName = async (puuid, first_name) => {
  try {
    const res = await pool.query(`UPDATE users SET first_name = $1 WHERE puuid = $2`, [first_name, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Update a user's last name by UUID
 * @param {String} puuid 
 * @param {String} last_name 
 * @returns 
 */
const AdminUpdateUserLastName = async (puuid, last_name) => {
  try {
    const res = await pool.query(`UPDATE users SET last_name = $1 WHERE puuid = $2`, [last_name, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
 ...
 * @returns 
 */
const AdminUpdateUserEmail = async (puuid, email) => {
  try {
    const res = await pool.query(`UPDATE users SET email = $1 WHERE puuid = $2`, [email, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Update a user's avatar by UUID
 * @param {String} puuid 
 * @param {String} bio 
 * @returns 
 */
const AdminUpdateUserBio = async (puuid, bio) => {
  try {
    const res = await pool.query(`UPDATE users SET bio = $1 WHERE puuid = $2`, [bio, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
 ...
 * @returns 
 */
const AdminUpdateUserGroup = async (puuid, user_group) => {
  try {
    const res = await pool.query(`UPDATE users SET user_group = $1 WHERE puuid = $2`, [user_group, puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Remove all webtokens of a user by UUID
 * @param {String} puuid 
 * @returns 
 */
const AdminDeleteAllWebtoken = async (puuid) => {
  try {
    const res = await pool.query(`DELETE FROM webtokens WHERE user_id = (SELECT id FROM users WHERE puuid = $1)`, [puuid]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Remove a singel webtoken of a user
 * @param {String} puuid 
 * @param {String} token 
 * @returns 
 */
const AdminDeleteWebtoken = async (puuid, token) => {
  try {
    const res = await pool.query(`DELETE FROM webtokens WHERE user_id = (SELECT id FROM users WHERE puuid = $1) AND token = $2`, [puuid, token]);
    return res;
  } catch (error) {
    throw new SQLError(error);
  }
...
/**
 * Get all stored membership data of a user by UserID
 * @param {Number} user_id 
 * @returns {Promise<Array>}
 */
const AdminGetMemberDataByUserID = async (user_id) => {
  try {
    const res = await pool.query(`SELECT * FROM user_membership WHERE user_id = $1`, [user_id]);
    return res.rows;
  } catch (error) {
    throw new SQLError(error);
  }
 ...
 * @param {Number} chart_hours
 * @param {Number} charts
 * @returns 
 */
const createServerMonitor = async (ipaddr, user_id, hostname, chart_hours, charts) => {
  console.log(ipaddr, user_id, hostname, chart_hours, charts);
  try {
    const res = await pool.query(
      `INSERT INTO netdata_monitoring (ip_address, user_id, hostname, chart_hours, charts) VALUES ($1, $2, $3, $4, $5)`,
      [ipaddr, user_id, hostname, chart_hours, charts]
    );
 ...
 */
const getServerMonitorDataCountByUserID = async (user_id, search) => {
  try {
    const res = await pool.query(
      `SELECT COUNT(*) FROM netdata_monitoring WHERE user_id = $1 AND hostname ILIKE $2`,
      [user_id, '%' + search + '%']
    );
...
/**
 * Get ip and hostname of a server monitor by user_id
 * @param {Number} user_id 
 * @param {Number} page
 * @param {Number} size
 * @param {String} search
 * @returns 
 */
const getServerMonitorDataByUserID = async (user_id, page, size, search) => {
  try {
    const res = await pool.query(
      `SELECT id, ip_address, hostname, charts FROM netdata_monitoring WHERE user_id = $1 AND hostname ILIKE $2 OFFSET $3 LIMIT $4`,
      [user_id, '%' + search + '%', page * size, size]
    );
...
/**
 * Get config parameters of a netdata monitor by id
 * @param {Number} id 
 * @returns 
 */
const getServerMonitorConfigByID = async (id) => {
  try {
    const res = await pool.query(`SELECT ip_address, hostname FROM netdata_monitoring WHERE id = $1`, [id])
    return res.rows
  } catch (error) {
    throw new SQLError(error)
  }
...
/**
 * Update the Project Activity Avatar
 * @param {String} puuid 
 * @param {String} avatar_url 
 * @returns 
 */
const EventUpdateAvatar = async (puuid, avatar_url) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET avatar_url = $1 WHERE puuid = $2`, [avatar_url, puuid]);
    return result
  } catch (err) {
 ...
 * @returns 
 */
const EventUpdateName = async (puuid, name) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET name = $1 WHERE puuid = $2`, [name, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns
 */
const EventUpdateColor = async (puuid, color) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET color = $1 WHERE puuid = $2`, [color, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateMinGroup = async (puuid, min_group) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET min_group = $1 WHERE puuid = $2`, [min_group, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @param {String} description
 */
const EventUpdateVisibility = async (puuid, visibility) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET visibility = $1 WHERE puuid = $2`, [visibility, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateDateApply = async (puuid, date_apply) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET date_apply = $1 WHERE puuid = $2 AND $1 < date_start`, [date_apply, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateDateStart = async (puuid, date_start) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET date_start = $1 WHERE puuid = $2 AND $1 < date_end`, [date_start, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateDateEnd = async (puuid, date_end) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET date_end = $1 WHERE puuid = $2 AND $1 > date_start`, [date_end, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateLocationAddress = async (puuid, location_address) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET location_address = $1 WHERE puuid = $2`, [location_address, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
 ...
 * @returns 
 */
const EventUpdateDescription = async (puuid, description) => {
  try {
    const result = await pool.query(`UPDATE projectactivities SET description = $1 WHERE puuid = $2`, [description, puuid]);
    return result
  } catch (err) {
    throw new SQLError(err);
  }
...
const checkProjectActivityHasOAuthClient = async (activity_id) => {
  try {
    ...
    const result = await pool.query(`SELECT * FROM oauth_apps WHERE id = (SELECT id FROM projectactivities WHERE puuid = $1)`, [activity_id]);
...
/**
 * Get top 10 newest announcements for a project activity
 * @param {String} activity_uuid 
 */
const GetProjectActivityAnnouncements = async (activity_uuid) => {
  const result = await pool.query(`SELECT * FROM projectactivity_timeline WHERE activity_id = (SELECT id FROM projectactivities WHERE puuid = $1) ORDER BY time DESC LIMIT 10`, [activity_uuid]);
  return result.rows;
}

/**
 * Create a new announcement for a project activity
 * @param {String} activity_uuid 
 * @param {String} type 
 * @param {String} data 
 * @returns 
 */
const createProjectActivityhAnnouncement = async (activity_uuid, type, data) => {
  const result = await pool.query(`INSERT INTO projectactivity_timeline (activity_id, content) VALUES ((SELECT id FROM projectactivities WHERE puuid = $1), $2)`, [activity_uuid, JSON.stringify({ type, data })]);
  return result;
}

/**
 * Delete a announcement for a project activity
 * @param {Number} activity_id
 * @param {String} timestamp 
 * @returns 
 */
const deleteProjectActivityAnnouncement = async (activity_id, timestamp) => {
  const result = await pool.query(`DELETE FROM projectactivity_timeline WHERE activity_id = $1 AND time = $2`, [activity_id, timestamp]);
  return result;
}
...
const deleteProjectActivityOAuthClient = async (activity_uuid) => {
  const result = await pool.query(`DELETE FROM oauth_apps WHERE id = (SELECT id FROM projectactivities WHERE puuid = $1)`, [activity_uuid]);
  return result;
}
...
const createProjectActivityOAuthClient = async (activity_uuid, name, client_id, client_secret, redirect_url, scope) => {
  ...
  try {
    ...
    const result = await client.query(`INSERT INTO oauth_apps (id, name, avatar_url, client_id, secret, redirect_url, scope) VALUES ((SELECT id FROM projectactivities WHERE puuid = $1), $2, $3, $4, $5, $6, $7)`, [activity_uuid, name, "/i/o", client_id, client_secret, redirect_url, scope]);
    await client.query('COMMIT');
    return result;
  } catch (err) {
    await client.query('ROLLBACK');
 ...
 * @returns 
 */
const WebtokensCreate = (user_id, token, browser, country) => {
  return new Promise((resolve, reject) => {
    pool.query(`INSERT INTO webtokens (user_id, token, browser, country) VALUES ($1, $2, $3, $4)`, [user_id, token, browser, country], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
/**
 * Get all Webtokens by a user ID
 * @param {Number} user_id 
 * @returns {Promise<Array>}
 */
const GetAllWebtokensByUserID = async (user_id) => {
  try {
    const result = await pool.query(`SELECT * FROM webtokens WHERE user_id = $1`, [user_id]);
    return result.rows;
  }
  catch (error) {
    new SQLError(error);
...
/**
 * Get all Webtokens by a user PUUID
 * @param {String} puuid 
 * @returns {Promise<Array>}
 */
const GetAllWebtokensByUserPUUID = async (puuid) => {
  try {
    const result = await pool.query(`SELECT * FROM webtokens WHERE user_id = (SELECT id FROM users WHERE puuid = $1)`, [puuid]);
    return result.rows;
  }
  catch (error) {
    new SQLError(error);
...
/**
 * Get the owner of a route by UUID
 * @param {String} uuid 
 * @param {String} owner_mode 
 * @returns 
 */
const getMiscUUIDRouteOwner = (uuid, owner_mode) => {
  return new Promise((resolve, reject) => {
    let dbQuery = '';
    switch (owner_mode) {
      // Project Activity
      case 'PA':
        dbQuery = `SELECT creator_user_id AS uuid_response FROM projectactivities WHERE puuid = $1`;
        break;
      default:
        reject('Invalid owner_mode');
        return;
    }
    pool.query(dbQuery, [uuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result.rows)
    })
  });
...
const user = {
  ...
  getWebtokens: GetWebtokensByUserID,
  deleteWebtoken: DeleteWebtokenByUUID,
  deleteAllOtherWebtokens: DeleteAllOtherWebtokens,
  permission: {
...
const admin = {
  users: {
    count_total: AdminGetTotalUsersCount,
    get_page: AdminGetUsersByPage,
    get_by_uuid: AdminGetUserByUUID,
    update: {
      username: AdminUpdateUserName,
      first_name: AdminUpdateUserFirstName,
      last_name: AdminUpdateUserLastName,
      email: AdminUpdateUserEmail,
      bio: AdminUpdateUserBio,
      user_group: AdminUpdateUserGroup,
    },
    delete: {
      webtoken: AdminDeleteWebtoken,
      webtoken_all: AdminDeleteAllWebtoken,
    }
  },
  member_data: {
    get_by_id: AdminGetMemberDataByUserID,
  }
}
...
const srvmon = {
  create: createServerMonitor,
  countByUserID: getServerMonitorDataCountByUserID,
  getByUserID: getServerMonitorDataByUserID,
  getConfigByID: getServerMonitorConfigByID
}
...
const projectactivities = {
  ...
  event: {
    ...
    get_announce: GetProjectActivityAnnouncements,
    add_announce: createProjectActivityhAnnouncement,
    del_announce: deleteProjectActivityAnnouncement
  },
  event_update: {
...
const webtoken = {
  ...
  getByUserID: GetAllWebtokensByUserID,
  getByUserPUUID: GetAllWebtokensByUserPUUID
}

const misc = {
  getUUIDRouteOwner: getMiscUUIDRouteOwner
}
