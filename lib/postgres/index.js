 ...
 * @returns 
 */
const createProjectActivityOAuthClient = async (activity_uuid, client_id, client_secret, redirect_url, scope) => {
  const client = await pool.connect();
  await client.query('BEGIN');
  try {
    // Insert the new OAuth Client
    const result = await client.query(`INSERT INTO oauth_apps (id, client_id, secret, redirect_url, scope) VALUES ((SELECT id FROM projectactivities WHERE puuid = $1), $2, $3, $4, $5)`, [activity_uuid, client_id, client_secret, redirect_url, scope]);
    await client.query('COMMIT');
    return result;
  } catch (err) {
    await client.query('ROLLBACK');
