 ...
 */
const GetCountAllEvents = (search) => {
  return new Promise((resolve, reject
  ) => {
    pool.query(`SELECT COUNT(*) FROM projectactivities WHERE type = 1 AND (name ILIKE $1)`, [search], (err, result) => {
      if (err) { reject(err) }
 ...
 * @returns 
 */
const GetAllEventByPage = (page, size, user_id, search) => {
  ...
  return new Promise((resolve, reject) => {
    pool.query(`SELECT projectactivities.*, CASE WHEN projectactivity_users.user_id IS NOT NULL THEN TRUE ELSE FALSE END AS user_participates
      ...
      WHERE 
        ...
        AND (name ILIKE $4)
      LIMIT 
      $1 OFFSET $2`, [size, offset, user_id, search], (err, result) => {
      if (err) { reject(err) }
      resolve(result.rows)
    })
  })
 ...
 * @param {String} puuid 
 */
const GetEventByUUID_Details = (puuid) => {
  return new Promise((resolve, reject) => {
    pool.query(`SELECT 
          projectactivities.*,
          BOOL_OR(projectactivity_users.user_id IS NOT NULL) AS user_participates, -- Aggregate check for any user participation
      ...
      GROUP BY 
        projectactivities.id`, [puuid], (err, result) => {
...
const EventUpdateName = (puuid, name) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET name = $1 WHERE puuid = $2`, [name, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateColor = (puuid, color) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET color = $1 WHERE puuid = $2`, [color, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateMinGroup = (puuid, min_group) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET min_group = $1 WHERE puuid = $2`, [min_group, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateVisibility = (puuid, visibility) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET visibility = $1 WHERE puuid = $2`, [visibility, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateDateApply = (puuid, date_apply) => {
  return new Promise((resolve, reject) => {
    // Only update if date_apply is before date_start
    pool.query(`UPDATE projectactivities SET date_apply = $1 WHERE puuid = $2 AND $1 < date_start`, [date_apply, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateDateStart = (puuid, date_start) => {
  return new Promise((resolve, reject) => {
    // Only update if date_start is before date_end
    pool.query(`UPDATE projectactivities SET date_start = $1 WHERE puuid = $2 AND $1 < date_end`, [date_start, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateDateEnd = (puuid, date_end) => {
  return new Promise((resolve, reject) => {
    // Only update if date_end is after date_start
    pool.query(`UPDATE projectactivities SET date_end = $1 WHERE puuid = $2 AND $1 > date_start`, [date_end, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateLocationAddress = (puuid, location_address) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET location_address = $1 WHERE puuid = $2`, [location_address, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
...
const EventUpdateDescription = (puuid, description) => {
  return new Promise((resolve, reject) => {
    pool.query(`UPDATE projectactivities SET description = $1 WHERE puuid = $2`, [description, puuid], (err, result) => {
      if (err) { reject(err) }
      resolve(result)
    })
  })
