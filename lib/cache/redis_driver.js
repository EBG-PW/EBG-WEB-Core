...
/**
 * Get the owner of a UUID of a given mode
 * @param {String} uuid 
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (cacheEntry) {
            ...
            if (dbEntry.length === 1) {
                ...
                await redis.set(`UUID:${owner_mode}:${uuid}`, JSON.stringify(dbEntry[0]), "EX", Number(process.env.UUIDCacheDurationS) * 1000 || 60_000);
                resolve(dbEntry[0]);
            } else {
