 ...
 * @param {Object} time 
 */
const addWebtoken = (webtoken, user_id, username, permissions, browser, language, design, time) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (inCache) {
            ...
            if (dbResult.length === 1) {
                ...
                const Formated_Permissions = mergePermissions(PermissionsResponse.rows, user_response[0].user_group); // Format the permissions to a array TODO: Add groups
                addWebtoken(token, dbResult[0].user_id, dbResult[0].username, Formated_Permissions, dbResult[0].browser, dbResult[0].language, dbResult[0].design, dbResult[0].time)
