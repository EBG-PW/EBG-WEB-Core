 ...
 * @returns 
 */
async function sendMail(emailType, emailResiver, waitForCompletion = false) {
    ...
    return new Promise((resolve, reject) => {
        const onCompleted = async (jobIdCompleted) => {
            if (jobIdCompleted === jobId) {
                mail_qEvents.off('completed', onCompleted); // Clean up event listener
                resolve();
            }
        };
        ...
        mail_qEvents.on('failed', (jobIdFailed, failedReason) => {
            if (jobIdFailed === jobId) {
                mail_qEvents.off('completed', onCompleted); // Clean up success listener
