const { user, misc, webtoken } = require('@lib/postgres');
 ...
 */
const addAutomaticStaticResponse = (routeID, oHash, type, data, statusCode, maxtime) => {
    redis.set(`ASR:${routeID}:${oHash}`, JSON.stringify({ type, data, time: new Date().getTime(), statusCode }), "EX", Math.ceil(maxtime / 1000));
}

/**
 * Get a ASR record from the cache but only returns it if it is not older than maxtime
 * @param {Number} routeID 
 * @param {String} oHash
 * @param {Number} maxtime 
 * @returns {Object|Boolean}
 */
const getAutomaticStaticResponseSave = (routeID, oHash, maxtime) => {
    return new Promise(async (resolve, reject) => {
        if (!routeID) return reject("No routeID provided");
        if (!maxtime) return reject("No maxtime provided");
        if (await redis.exists(`ASR:${routeID}:${oHash}`)) {
            // Redis deletes the key itself
            const storedItem = JSON.parse(await redis.get(`ASR:${routeID}:${oHash}`));
            resolve(storedItem);
        } else {
            resolve(false);
        }
...
module.exports = {
    ...
    addAutomaticStaticResponse: addAutomaticStaticResponse,
    getAutomaticStaticResponseSave: getAutomaticStaticResponseSave,
    IPLimit: IPLimit,
    IPCheck: IPCheck,
