...
const plublicStaticCache = (duration, objOptions = []) => {
    return async (req, res) => {
        try {
            ...
            if (objOptions.length === 0) throw new CustomError(`No object options provided for ${req.route.pattern}`).withStatus(500).withInfo("Can´t generate cache key");
            ...
            const cachHash = oHash(importantDataObject, { algorithm: process.env.INTERNAL_HASH || "sha3-256" })
